...
Double_t* DoClusure(TTree* t, Double_t p1, Double_t p2, TString fs, TString year, bool isData);
void MakePlot(Double_t* x_corr, Double_t* x_uncorr, Double_t* y, Double_t* yErr, TString fs, TString year, bool isData);
...
void Closure2Lepton(){
	...
	TFile* f = new TFile();
         ...
         if(isData==0&&year=="2016"&&fs=="muon")f = new TFile("/raid/raid9/chenguan/input/AfterSync_200424/2016_DY_madg_m2mu.root");
         if(isData==0&&year=="2017"&&fs=="muon")f = new TFile("/raid/raid9/chenguan/input/AfterSync_200424/2017_DY_madg_m2mu.root");
         if(isData==0&&year=="2018"&&fs=="muon")f = new TFile("/raid/raid9/chenguan/input/AfterSync_200424/2018_DY_madg_m2mu.root");
         if(isData==0&&year=="2016"&&fs=="electron")f = new TFile("/raid/raid9/chenguan/input/AfterSync_200424/2016_DY_madg_m2e.root");
         if(isData==0&&year=="2017"&&fs=="electron")f = new TFile("/raid/raid9/chenguan/input/AfterSync_200424/2017_DY_madg_m2e.root");
         if(isData==0&&year=="2018"&&fs=="electron")f = new TFile("/raid/raid9/chenguan/input/AfterSync_200424/2018_DY_madg_m2e.root");
 
         if(isData==1&&year=="2016"&&fs=="muon")f = new TFile("/raid/raid9/chenguan/input/Vertex_AfterSync_200424/SingleMuon2016_m2mu.root");
         if(isData==1&&year=="2017"&&fs=="muon")f = new TFile("/raid/raid9/chenguan/input/Vertex_AfterSync_200424/SingleMuon2017_m2mu.root");
         if(isData==1&&year=="2018"&&fs=="muon")f = new TFile("/raid/raid9/chenguan/input/Vertex_AfterSync_200424/SingleMuon2018_m2mu.root");
         if(isData==1&&year=="2016"&&fs=="electron")f = new TFile("/raid/raid9/chenguan/input/Vertex_AfterSync_200424/crab_SingleElectron_2016_vtx_m2e.root");
         if(isData==1&&year=="2017"&&fs=="electron")f = new TFile("/raid/raid9/chenguan/input/Vertex_AfterSync_200424/crab_SingleElectron_2017_vtx_m2e.root");
         if(isData==1&&year=="2018"&&fs=="electron")f = new TFile("/raid/raid9/chenguan/input/Vertex_AfterSync_200424/crab_SingleElectron_2018_vtx_m2e.root");
	...
	for(Int_t i=0; i<10; i++){
		tmp = DoClusure(t, cut[i], cut[i+1], fs, year, isData);
		x_corr[i] = tmp[0];
		x_uncorr[i] = tmp[1];
		y[i] = tmp[2];
		yErr[i] = tmp[3];
	...
	MakePlot(x_corr, x_uncorr, y, yErr, fs, year, isData);
...
void MakePlot(Double_t* x_corr, Double_t* x_uncorr, Double_t* y, Double_t* yErr, TString fs, TString year, bool isData){
	...
	TString plotpath;
	if(isData==1)plotpath = "/home/chenguan/public_html/Closure2Lepton/Data/";
	if(isData==0)plotpath = "/home/chenguan/public_html/Closure2Lepton/MC/";
	TString filename_png = fs+"_ClosureTest"+"_"+tag+".png";
	TString filename_pdf = fs+"_ClosureTest"+"_"+tag+".pdf";
	c->SaveAs(plotpath+year+"/"+filename_pdf);
	c->SaveAs(plotpath+year+"/"+filename_png);
...
//	latex1->DrawLatex(0.3,0.95,"CMS Preliminary");
	
	TString p1_s, p2_s;
	p1_s = to_string(p1);
	p2_s = to_string(p2);
	TString plotpath;
	if(isData==1)plotpath = "/home/chenguan/public_html/Closure2Lepton/Data/"+year+"/"+fs+"_Relative_massZErr_"+p1_s+"-"+p2_s;
	if(isData==0)plotpath = "/home/chenguan/public_html/Closure2Lepton/MC/"+year+"/"+fs+"_Relative_massZErr_"+p1_s+"-"+p2_s;
