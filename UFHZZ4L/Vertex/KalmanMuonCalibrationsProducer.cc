...
KalmanMuonCalibrationsProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   for(unsigned i = 0 ; i < nbMuon; i++){
       ...
       int mu_genparticle_roc = 0;
       double mu_gen_pt_roc = 0;
       int mu_nl_roc = 0;
	   ...
	   mu_nl_roc = nl;//this line works for data and MC
		   ...
		   mu_genparticle_roc = 1;
		   mu_gen_pt_roc = gen_particle->pt();
               }
               else
               {
           ...
           else if(!isMC && nl > 5)//if here is data(isMC==0) do not store mu_genparticle_roc and mu_gen_pt_roc
           {
	   ...
	   mu.addUserFloat("scale_sf", scale_factor);
	   mu.addUserFloat("mu_pt_roc",oldpt);
	   mu.addUserFloat("mu_eta_roc",mu.eta());
	   mu.addUserFloat("mu_phi_roc",mu.phi());
	   mu.addUserFloat("mu_q_roc",mu.charge());
	   mu.addUserFloat("mu_tracktype_roc",mu.muonBestTrackType());
	   mu.addUserFloat("mu_nl_roc",mu_nl_roc);
	   mu.addUserFloat("mu_genparticle_roc",mu_genparticle_roc);
	   mu.addUserFloat("mu_gen_pt_roc",mu_gen_pt_roc);
