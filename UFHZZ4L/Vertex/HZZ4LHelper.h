...
public:
    ...
    std::vector<pat::Muon> goodLooseMuons2012(edm::Handle<edm::View<pat::Muon> > Muons, edm::Handle<edm::View<pat::Muon> > MuonsUnS, double muPtCut);
...
std::vector<pat::Muon> HZZ4LHelper::goodLooseMuons2012(edm::Handle<edm::View<pat::Muon> > Muons, edm::Handle<edm::View<pat::Muon> > MuonsUnS, double muPtCut) {
	using namespace pat;
	using namespace std;
	vector<pat::Muon> bestMuons;
	vector<bool> Mu_passLoose;
	for(edm::View<pat::Muon>::const_iterator mu=Muons->begin(); mu != Muons->end(); ++mu) {
		if( (mu->isGlobalMuon() || mu->isTrackerMuon() || mu->isPFMuon()) && fabs(mu->eta()) < 2.4 && mu->pt() > muPtCut) {
			Mu_passLoose.push_back(true);
		}
		else Mu_passLoose.push_back(false);
	}
	int i = 0;
	for(edm::View<pat::Muon>::const_iterator mu=MuonsUnS->begin(); mu != MuonsUnS->end(); ++mu) {
		if(Mu_passLoose[i]) {
