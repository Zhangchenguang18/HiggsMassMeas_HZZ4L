...
Double_t massZErr, eta1, eta2, pT1, pT2, pterr1, pterr2, m1, m2, phi1, phi2, RelIso1, RelIso2, lep1_precorr, lep2_precorr; Int_t lep1_ecalDriven, lep2_ecalDriven;
TString year = "2018";
TString fs = "electron";
TString plots = "";
...
void GetElectronScaleShift(){	
	...
	for(Int_t i=0; i<3; i++){//eta loop 
		for(Int_t j=0; j<6; j++){//pt loop
			...
			result[0] = (tmp_data[0]-tmp_mc[0])/(91.19+tmp_mc[0]);
//			cout<<"old "<<(tmp_data[0]-tmp_mc[0])/91.19<<endl;
//			cout<<"new "<<(tmp_data[0]-tmp_mc[0])/(91.19+tmp_mc[0])<<endl;
			result[1] = sqrt(tmp_mc[1]*tmp_mc[1]+tmp_data[1]*tmp_data[1])/(91.19+tmp_mc[0]);
...
void ClosurePlot(vector<Double_t> &results){
	...
	g1->GetYaxis()->SetTitle("(m_{data}-m_{MC})/m_{MC}");
	g1->GetXaxis()->SetLimits(0,100);
	g1->GetHistogram()->SetMaximum(0.007);
	g1->GetHistogram()->SetMinimum(-0.007);
...
//		cout<<"2ed n1 "<<n1.getVal()<<endl;
	...
	c.SaveAs("/home/chenguan/public_html/Syst_uncertainty/LepEnergyScale/"+year+"/ScaleShift/Z/electron/"+subname+name+".png");
//	c.SaveAs("/home/chenguan/public_html/Syst_uncertainty/LepEnergyScale/"+year+"/ScaleShift/Z/electron/"+subname+name+".pdf");
...
/*	//HIG-16-041
	...
	*/
	...
	//Run2 Legacy
	if(isData==0&&year=="2016"&&fs=="electron")f = new TFile("/raid/raid9/chenguan/input/Vertex_AfterSync_200424/2016DY_m2e.root");
	if(isData==0&&year=="2017"&&fs=="electron")f = new TFile("/raid/raid9/chenguan/input/Vertex_AfterSync_200424/2017DY_m2e.root");
	if(isData==0&&year=="2018"&&fs=="electron")f = new TFile("/raid/raid9/chenguan/input/Vertex_AfterSync_200424/2018DY_m2e.root");
	...
	for(Int_t i=0; i<sum; i++){
		...
		TLorentzVector l1,l2;
                ...
                l2.SetPtEtaPhiM(pT2,eta2,phi2,m2);
                double massZ = (l1+l2).M();
	//	cout<<lep1_precorr<<"  "<<l1.Energy()<<endl;
...
//	t->SetBranchAddress("pt1_eleuncorr",&pT1);//for RunI and HIG-16-041 style, use uncorr variable 
//	t->SetBranchAddress("pt2_eleuncorr",&pT2);
	t->SetBranchAddress("pT1",&pT1);
	t->SetBranchAddress("pT2",&pT2);
	t->SetBranchAddress("pterr1",&pterr1);
	t->SetBranchAddress("pterr2",&pterr2);
//	t->SetBranchAddress("mass1_eleuncorr",&m1);
//	t->SetBranchAddress("mass2_eleuncorr",&m2);
...
//	t->SetBranchAddress("lep1_precorr",&lep1_precorr);
//	t->SetBranchAddress("lep2_precorr",&lep2_precorr);
...
//	c.SaveAs("/home/chenguan/public_html/Syst_uncertainty/LepEnergyScale/"+year+"/ScaleShift/LUT.pdf");
